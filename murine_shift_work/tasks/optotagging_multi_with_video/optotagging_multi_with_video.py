...
def run_task(**args_dict):
    ...
    for sub_task_name, sub_task_settings in all_sub_task_settings.items():
        ...
        with TaskProcess(**patched_arg_dict) as tp:
            # Video
            conductor_args = {
                "config_file": patched_arg_dict["config_file_camera"],
                "acquisition_group": patched_arg_dict["is_child_session_to"]
                if patched_arg_dict["is_child_session_to"] is not None
            ...
            # video_stream_config = patched_arg_dict["settings.camera"].get(
            #     "controllers", {}
            # )
