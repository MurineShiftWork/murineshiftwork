...
def _add_connection(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument("-ip", "--remote-ip", dest="ip", default=_DEFAULT_IP,
                   help="Open Ephys GUI host IP")
    g.add_argument("-port", "--remote-port", dest="port", type=int, default=_DEFAULT_PORT,
                   help="Open Ephys HTTP API port")
    g.add_argument("-d", "--debug", dest="debug", action="store_true", default=False,
                   help="Enable debug logging")


def _add_metadata(parser: argparse.ArgumentParser) -> None:
    g = parser.add_argument_group("Metadata / paths")
    g.add_argument("--local-path", dest="local_path", default=_DEFAULT_LOCAL_PATH,
                   help="Local base data directory")
    g.add_argument("--remote-path", dest="remote_path", default=_DEFAULT_REMOTE_PATH,
                   help="Remote base data directory (as seen by Open Ephys GUI)")
    g.add_argument("--subject", dest="subject", default="_test_oe_controller",
                   help="Subject identifier")
    g.add_argument("--session-extension", dest="session_extension", required=True,
                   help="Session label appended to subject__datetime (e.g. pxi, intan_ttl)")
    g.add_argument("--acquisition-extension", dest="acquisition_extension", default="",
                   help="Acquisition label for the parent folder (enables parent mode). "
                        "Example: ephys_multi_behavior")
    g.add_argument(
        "--child", "--is-child-session-to", dest="is_child_session_to", default="",
        metavar="ACQ_PATH",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # status
    p = sub.add_parser("status", formatter_class=_Fmt,
                        help="Print current GUI mode (IDLE / ACQUIRE / RECORD)")
    _add_connection(p)
    p.set_defaults(func=cmd_status)

    # preview
    p = sub.add_parser("preview", formatter_class=_Fmt,
                        help="Start acquisition (ACQUIRE mode, no recording)")
    _add_connection(p)
    p.set_defaults(func=cmd_preview)

    # record
    p = sub.add_parser("record", formatter_class=_Fmt,
    ...
    # stop
    p = sub.add_parser("stop", formatter_class=_Fmt,
                        help="Stop acquisition / recording (IDLE mode)")
    _add_connection(p)
