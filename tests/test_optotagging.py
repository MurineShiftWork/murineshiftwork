...
from murineshiftwork.tasks.optotagging.task import OptoTaggingRecord
...
def _make_task_for_video(session_basename, subject, is_child=None):
    from murineshiftwork.tasks.optotagging.task import Task
...
def test_start_protocol_video_path():
    ...
    conductor.initialize_acquisition.assert_called_once_with(
        acquisition_path=str(Path(rel) / "sess_001__proto_40hz"),
