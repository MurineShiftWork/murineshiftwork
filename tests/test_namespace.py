...
def test_v1_basename_structure():
    paths = generate_session_paths("mouse_01", "flush", "/data", version=NAMESPACE_V1, printout=False)
...
def test_legacy_datetime_seconds_only():
    paths = generate_session_paths("mouse_01", "flush", "/data", version=NAMESPACE_LEGACY, printout=False)
    dt_str = paths["session_basename"].split("__")[1]
...
def test_unknown_version_raises():
    with pytest.raises(ValueError, match="Unknown namespace version"):
        generate_session_paths("mouse_01", "flush", "/data", version="v99", printout=False)


def test_forbidden_chars_in_subject_raise():
    for bad_char in [" ", "#", "@", "!"]:
        with pytest.raises(ValueError, match="forbidden characters"):
            generate_session_paths(f"mouse{bad_char}01", "flush", "/data", printout=False)


def test_test_task_uses_default_subject():
    paths = generate_session_paths(
        "real_subject", "_test__flush", "/data",
        version=NAMESPACE_V1,
        printout=False,
    )
...
def test_build_data_paths_uses_current_version():
    paths = build_data_paths(basepath="/data", subject="mouse_01", task="flush", printout=False)
...
def test_build_data_paths_output_equivalent_to_generate():
    ...
    a = build_data_paths(basepath="/data", subject="mouse_01", task="flush", printout=False)
