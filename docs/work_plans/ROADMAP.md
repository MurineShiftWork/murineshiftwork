...
- [ ] **`msw-oe` plugin package** — separate repo, registers `msw oe status` (show OE base_text + status) via `msw.cli` entry-point group; plugin contract already live in MSW; design in `MASTER_PLAN §5`.
...
- [ ] **Valve calibration fallback + staleness warning** — three-tier logic in `evaluate.py` `_resolve_setup_config_ports`: (1) setup has calibration for port → use it; (2) setup `bpod_valve` dict is empty → warn loudly, use per-family anchor default (`standard` ≈ 2 µL@28ms, `npx` ≈ 13 µL@102ms); (3) setup has *some* valve calibration but this port is missing → fail hard (misconfiguration, not absence). Staleness check: per-valve `WARNING` if `updated` > 180 days ago (constant in `models.py`). Setup YAML gains optional `valve_family: standard | npx` (default `standard`). Anchor `ValveCalibration` defaults live in `models.py`. Design approved 2026-05-25.
