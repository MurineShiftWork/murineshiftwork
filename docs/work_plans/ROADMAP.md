...
- [ ] **GitHub: create repo + PyPI release for `rpi-camera-ensemble`** — push `external/provision_rpi/rpi_camera_ensemble`; ensure it is publicly installable (coexists with FLIR/Bonsai backend). Toolchain + pre-commit fully passing as of 2026-05-26; three commits pending locally before push.
...
- [ ] **URGENT — Retrograde reader Phase 2: `fixture_legacy` + reader dispatch refactor** — (a) copy `/ceph/sjones/users/lars/data/s008_tab_m1113182_LR/s008_tab_m1113182_LR__20210707_143042__probabilistic_switching/` to `tests/data/fixture_legacy/`, add one line to `FIXTURE_EXPECTATIONS`; (b) split `read_session_data()` into `_read_session_yaml` / `_read_separate_json` / `_read_legacy` with `_READER_DISPATCH` dict; (c) expose `artifact_format` + `namespace_version` at top level of return dict. See `PLAN_retrograde_reader.md §Phase 2`.
...
- [ ] **URGENT FIX — namespace wiring into OE parent session paths** — verify that `OpenEphysParentSession.attach()` path assembly uses `NamespaceBuilder` / `extract_level_values` + `build_path` roundtrip correctly for the acquisition-name component; specifically check that `base_text` parsing in `parent_session.py` does NOT assume Windows-style separators. OE `base_text` is set by `oe_remote` using forward slashes only (`subject/acquisition_name/oe_session`) — a backslash separator would silently break the split and produce a wrong `acquisition_name`. See MASTER_PLAN §5.
- [ ] **URGENT FIX — `session.py`: `is_complete_session` always False for v2+ sessions** — `required_file_keys` includes `"raw"` but v2+ sessions never write a raw CSV; remove `"raw"` from required keys. Fix in `src/murineshiftwork/readers/session.py` (line ~117). Causes `test_read_session_data_complete` to fail.
- [ ] **URGENT FIX — `validate.py`: RCESession pydantic model-rebuild error** — `RCESession` from `rpi_camera_ensemble.io.session` is not fully defined at import time (pydantic v2 deferred annotations issue); call `RCESession.model_rebuild(_types_namespace={"Path": Path})` after import in `_check_rce_completeness()`. Causes `test_validate_jsonl_passes` and `test_validate_pkl_passes` to fail even for non-RCE fixture sessions.
...
- [ ] **rpi-camera-ensemble: build system migration** — setuptools → hatchling + src/ layout; deferred from 2026-05-26 toolchain session (pre-commit now passing, three commits pending); do after GitHub repo is created and first push is in; see `PLAN_repo_upgrades.md §rpi_camera_ensemble`
...
- [x] **MSW Monitor — Step 1: LogAgent + ingest server** — `logagent/logagent.py` (daemon Process, bearer token, session UUID, three-phase lifecycle), `logagent/server.py` (FastAPI ingest endpoints); `TaskProcess._start_relay()`; `session_uuid` in session YAML; `put_nowait()` in sequence task · 2026-05-23 · `ft/monitor-step1`
