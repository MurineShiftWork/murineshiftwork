...
parser = argparse.ArgumentParser(
    description="Benchmark for testing video performance"
)
parser.add_argument(
    "--cuda",
    default=False,
...
parser.add_argument(
    ...
    default=3,
    type=int,
    help="Number of image frames to generate (default=3)",
)
parser.add_argument(
    "--image-mode",
    default="mono",
    choices=["mono", "rgb"],
    help="Image data mode (mono or rgb)",
    dest="image_mode",
)
parser.add_argument(
    "--levels",
...
parser.add_argument(
    "--lut", default=False, action="store_true", help="Use color lookup table"
)
parser.add_argument(
    "--lut-alpha",
    default=False,
    action="store_true",
    help="Use alpha color lookup table",
    dest="lut_alpha",
)
parser.add_argument(
    "--size",
...
if args.levels is None:
    ...
    if len(args.levels) == 2:
        ...
        raise ValueError(
            "levels argument must be 2 or 6 comma-separated values (got %r)"
            % (args.levels,)
        )
...
def mkData():
    with pg.BusyCursor():
        ...
        cacheKey = (
            ui.dtypeCombo.currentText(),
            ui.rgbCheck.isChecked(),
            frames,
            width,
            height,
        )
        if cacheKey not in cache:
            ...
            for idx in range(view.shape[0]):
                subdata = xp.random.normal(
                    loc=loc, scale=scale, size=chan_shape
                )
...
def updateSize():
    ...
    ui.sizeLabel.setText(
        "%d MB" % (frames * width * height * rgb * dtype.itemsize / 1e6)
    )
...
cap = cv2.VideoCapture("http://192.168.100.31:9999")


def update():
    global ui, ptr, lastTime, fps, LUT, img, cap
    ...
    if ui.rawRadio.isChecked():
        ui.rawImg.setImage(data, lut=useLut, levels=useScale)
        ui.stack.setCurrentIndex(1)
    elif ui.rawGLRadio.isChecked():
        ui.rawGLImg.setImage(data, lut=useLut, levels=useScale)
